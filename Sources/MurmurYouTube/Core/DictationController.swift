...
final class DictationController {
    ...
    @discardableResult
    func activate() -> Bool {
        hotkey.shortcut = Settings.shared.pushToTalkShortcut
    ...
    // MARK: - Hybrid Hotkey Handling (Hold to Talk or Tap to Toggle)
    ...
    private func handleHotkeyPress() {
        ...
        if state == .listening && isLockedRecording {
            // Second tap during toggle recording finishes dictation!
        ...
        if state.isActive {
            // Already active, ignore repeat triggers
            return
        }
    ...
    private func handleHotkeyRelease() {
        ...
        // If the key was tapped quickly (< 350ms), lock into toggle mode so it stays recording!
        if duration < 0.35 && state.isActive {
            isLockedRecording = true
            Log.hotkey.info("short tap detected (\(duration, privacy: .public)s) — lock-recording in toggle mode")
            return
        }

        // Key was held down (>= 350ms) — traditional push-to-talk release
        isLockedRecording = false
        endDictation()
    }
    ...
    /// Convenient single-action toggle for modern UI buttons.
    ...
    private func beginDictation() {
        ...
        Task { @MainActor in
            do {
                ...
                // If user released during spin-up, gracefully transition rather than aborting.
                if self.pendingStop {
    ...
    private func endDictation() {
        ...
        // If still starting up, defer the stop until the engine and tap are ready.
        ...
        Task { @MainActor in
            ...
            let currentConsume = self.consumeTask

            await withTaskGroup(of: Void.self) { group in
                group.addTask {
                    await currentEngine?.finish()
                    await currentConsume?.value
                }
                group.addTask {
