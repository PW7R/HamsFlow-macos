...
@main
struct HamsFlowApp: App {
    @NSApplicationDelegateAdaptor(AppDelegate.self) private var delegate

    var body: some Scene {
        Window("HamsFlow", id: "main") {
        ...
        .commands {
            ...
            CommandGroup(replacing: .appSettings) {
                Button("Settings…") {
                    SettingsWindowManager.shared.show(controller: delegate.controller)
                }
                .keyboardShortcut(",")
            }
        ...
        SwiftUI.Settings {
            ModernSettingsView(controller: delegate.controller)
        }
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        observeState()
        Log.app.info("HamsFlow ready — hold or tap \(Settings.shared.pushToTalkShortcut.displayName) to dictate")
    }

    /// `hamsflow://clear`, `hamsflow://show`, `hamsflow://settings`
    func application(_ application: NSApplication, open urls: [URL]) {
        for url in urls where url.scheme == "hamsflow" || url.scheme == "murmuryt" {
            switch url.host {
            case "clear":
                RunLog.clear()
...
private struct MenuContent: View {
    ...
    var body: some View {
        Text("Hold \(settings.pushToTalkShortcut.displayName) to dictate")
        ...
        Picker("Push-to-talk key", selection: Binding(
            ...
            ForEach(PushToTalkShortcut.presets, id: \.self) { shortcut in
                Text(shortcut.displayName).tag(shortcut)
            }
            if !PushToTalkShortcut.presets.contains(settings.pushToTalkShortcut) {
                Text("Custom: \(settings.pushToTalkShortcut.displayName)").tag(settings.pushToTalkShortcut)
            }
        }
